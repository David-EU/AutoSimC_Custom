...
if __name__ == "__main__":
    try:
        filename = "settings_compare.py"
        source = open(filename, 'r').read()
        compile(source, filename, 'exec')
    except SyntaxError:
        print("Error in settings_compare.py, pls check for syntax-errors")
        sys.exit(1)
...
def parse_command_line_args():
    ...
    parser.add_argument('-sim',
                        ...
                        "Edit the simcraft-path in settings_compare.py to point to your simc-installation. The result.html "
                        ...
                        "You have to set the simc path in the settings_compare.py file."
                        ...
                        "or instabilities, edit settings_compare.py and change the corresponding parameters or even disable it."
                        )
...
def validateSettings(args):
    """Check input arguments and settings_compare.py options"""
        ...
        if os.name == "nt":
            if not settings.simc_path.endswith("simc.exe"):
                raise RuntimeError("Simc executable must end with 'simc.exe', and '{}' does not."
                                   "Please check your settings_compare.py simc_path options.".format(settings.simc_path))
    ...
    if outputGenerated:
        if num_generated_profiles == 0:
            raise ValueError("No valid profile combinations found."
                             " Please run again with --debug and check your input.txt and settings_compare.py.")
        if args.sim:
            if not settings.skip_questions:
